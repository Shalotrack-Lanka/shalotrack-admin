...
<div class="flex h-screen overflow-hidden">
    ...
    <div class="flex-1 flex flex-col overflow-y-auto">
        ...
        <main class="p-4 md:p-6 flex-1 bg-gray-50" x-data="cancelSimPage()">

            <div class="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden w-full">

                <div class="px-6 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
                    <h3 class="text-lg font-bold text-gray-800">Cancel Sim</h3>
                    <button
                        @click="refresh()"
                        :disabled="refreshing"
                        class="flex items-center gap-2 bg-gray-100 hover:bg-gray-200 disabled:opacity-50 text-gray-700 text-sm font-semibold px-4 py-2 rounded-lg shadow-sm transition">
                                ...
                                <tr>
                                    ...
                                    <td class="p-3 text-center">
                                        <button
                                            ...
                                            <span x-show="!sim.saving">Save</span>
                                            <span x-show="sim.saving">...</span>
                                        </button>
                                    </td>
                                </tr>
                            </template>

                            <tr x-show="sims.length === 0">
                                <td colspan="7" class="p-6 text-center text-gray-400 font-semibold">No SIMs found.</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
...
function cancelSimPage() {
    return {
        ...
        statusClass(status) {
            ...
            if (status === 'Faulty') return 'bg-orange-100 text-orange-700';
        ...
        async refresh() {
            ...
            try {
                ...
                this.showToast('Failed to refresh. Check the console.', 'error');
                console.error(e);
            } finally {
        ...
        async save(sim) {
            ...
            try {
                const res = await fetch(`/admin/master-pages/cancel-sim/${sim.id}`, {
                    method: 'PATCH',
                    headers: {
                ...
                const data = await res.json();
                sim.canceled_at = data.canceled_at;

                this.showToast(`${sim.sim_number} updated successfully.`, 'success');
            } catch (e) {
