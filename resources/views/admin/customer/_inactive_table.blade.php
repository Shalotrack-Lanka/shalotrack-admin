<div class="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden w-full">
    <div class="px-6 py-4 border-b border-gray-100 bg-red-50 flex items-center justify-between flex-wrap gap-3">
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('admin.customer-setup.report', ['type' => 'inactive']) }}" target="_blank"
               class="bg-red-600 hover:bg-red-700 text-white text-xs font-bold px-3 py-1.5 rounded-lg shadow-sm transition inline-flex items-center gap-1">
        ...
        <table class="w-full text-left text-xs border-collapse">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50 transition">
                    <td class="p-3 font-bold text-slate-800">{{ $c->full_name }}</td>
                            ...
                            <div class="space-y-1">
                                @foreach($c->vehicles as $v)
                                    <div class="flex items-center gap-1.5 text-[11px]">
                                        ...
                                        <span class="text-slate-400">{{ $v->make }} {{ $v->model }}</span>
                                        @if($v->has_gps_device)
                    ...
                    <td class="p-3">
                        @if(isset($c->dealerLead) && $c->dealerLead)
                            <div class="text-[11px]">
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full bg-cyan-50 text-cyan-700 border border-cyan-200 text-[10px] font-bold">
                                    Via Dealer
                                </span>
                                <div class="mt-0.5 font-bold text-slate-600">{{ $c->dealerLead->dealer->full_name ?? '—' }}</div>
                            </div>
                        @else
                ...
                @empty
                <tr>
                    <td colspan="7" class="p-6 text-center text-gray-400">No inactive customers.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
